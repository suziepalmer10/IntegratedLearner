...
credint.learner <- function(fit,test=FALSE,ylab=NULL,xlab="Observations",
                            cex.main=1,font.main=1,cex.lab=1,cex.axis=1,
                            style="plain",legend=c("Y=0", "Y=1"),...){
  
  if(fit$base_learner =="SL.BART" & fit$meta_learner=="SL.nnls.auc"){
    weights <- fit$weights
    
    if(test==TRUE){
      if(fit$test==FALSE){stop("No test set information available as part of fit object")}
