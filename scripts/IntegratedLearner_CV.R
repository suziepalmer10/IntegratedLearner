...
library(caret)
# library(glmnetUtils)
# library(mxBART) # One should install this package before running it
# library(bayesreg)
library(cowplot)
library(pROC)
library(ROCR)
...
run_integrated_learner_CV<-function(feature_table,
                                    ...
                                    base_screener = 'All', # Whether to screen variables before fitting base models ? All means no screening
                                    meta_learner = 'SL.nnls.auc', # Meta learner for stacked generalization.
                                    ...
                                    verbose = FALSE, # Should detailed message be printed? Default is FALSE.
                                    print_learner = TRUE, # Should a detailed summary be printed? Default is TRUE.
                                    refit.stack = TRUE, # Report post-refit predictions on entire data for the stacked layer? Default is TRUE.
                                    family=gaussian())
{ 
  ...
  if (family$family=='gaussian' && length(unique(sample_metadata$Y)) <= 5) {
    warning("The response has five or fewer unique values.  Are you sure you want family to be gaussian?")
  }
  ...
  if(run_concat & run_stacked){
    ...
    if(refit.stack){
      yhat.train <- cbind(combo, SL_fit_stacked$SL.predict, SL_fit_concat$Z)
    } else{
    ...
    if(refit.stack){
      yhat.train <- cbind(combo, SL_fit_stacked$SL.predict)
    } else{
